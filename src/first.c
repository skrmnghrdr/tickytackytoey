...
int testfunct(){
    printf("Well shit, how come this one workds...\n");
    return 0;
}


void clear(){
  //clear does not really clear the buffer, it just scrolls all the way down?
  debug("Clearing\n");
  printf("\033[2J\033[1;1H");
}

int check_stale(int marked_coordinates){
  debug("check_stale ran.\n");
  printf("%d\n", marked_coordinates);
  if(marked_coordinates == max_marked_coordinates){
    debug("check_stale == max_marked.\n");
    return 1;
  }
  return 0;
}
void debug(char *debug_info){
  /* prints if debug_info == 1 */
  if(debug_info){
    printf("%s", debug_info);
  }
}
